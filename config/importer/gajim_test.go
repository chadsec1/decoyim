...
func (s *GajimSuite) Test_GajimImporter_canImportFingerprintsFromFile(c *C) {
	...
	c.Assert(res[0].Fingerprint, DeepEquals, decode("57d8ea36c76d5d800fe790c56dc33feb254e899b"))
	c.Assert(res[0].Untrusted, Equals, true)

	c.Assert(res[1].UserID, Equals, "coyim@thoughtworks.com")
	c.Assert(res[1].Fingerprint, DeepEquals, decode("c8123327e389e3d036ba91cf92d722f515057b61"))
	c.Assert(res[1].Untrusted, Equals, true)

	c.Assert(res[2].UserID, Equals, "someone@where.com")
	c.Assert(res[2].Fingerprint, DeepEquals, decode("a334e9d582da18f15028f7f7412bc8d15d0a1558"))
	c.Assert(res[2].Untrusted, Equals, true)

	c.Assert(res[3].UserID, Equals, "bla@rose.com")
	c.Assert(res[3].Fingerprint, DeepEquals, decode("7c6c74ddb307c95fa30c3ecab25ee64a54124447"))
	c.Assert(res[3].Untrusted, Equals, false)

	c.Assert(res[4].UserID, Equals, "not@coyim.com")
	c.Assert(res[4].Fingerprint, DeepEquals, decode("4157eea3bb3cf86cc0379e4c270e89b976bc34da"))
	c.Assert(res[4].Untrusted, Equals, true)

	c.Assert(res[5].UserID, Equals, "not@coyim.com")
	c.Assert(res[5].Fingerprint, DeepEquals, decode("edd6274423cd2fb6993da928d923075be2d0d52a"))
	...
	c.Assert(*res.Accounts[1], DeepEquals, config.Account{
		...
		Peers: []*config.Peer{
			...
			},
			{
				...
				Fingerprints: []*config.Fingerprint{
					{
						Fingerprint: decode("7c6c74ddb307c95fa30c3ecab25ee64a54124447"),
						Trusted:     true,
					},
				},
			},
